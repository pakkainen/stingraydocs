...
Удобный интерфейс для формирования строки запуска сканирования, которая может использоваться во внешнем CI/CD-конвейере, например с целью интеграции **Stingray** с различными системами дистрибуции. Непосредственно интеграция осуществляется посредством специально разработанного средства инструментальной поддержки — [mdast-cli](https://github.com/Dynamic-Mobile-Security/mdast-cli).

Как было сказано в разделе «[Запуск сканирования](../ug/zapusk_skanirovaniya.md)», сканирование приложения можно запустить не только из пользовательского интерфейса системы, но также из командной строки. Запуск сканирования из командной строки можно производить только имея валидный токен.
...
* Выберите тест-кейс, который будет выполняться в рамках автоматического сканирования.
...
5. Укажите пароль и код двухфакторной аутентификации в поле **Пароль 2FA**.
    ...
    !!! note "Примечание"
        Для получения кода двухфакторной аутентификации попробуйте, например, залогиниться на iCloud.com. Очевидно, что двухфакторная аутентификация для аккаунта должна быть включена.
...
4. Для успешной интеграции необходимо указать следующие параметры:
    ...
    * `Auth Sub Token`.
    ...
    !!! note "Примечание"
        Порядок получения `GSF ID` и `Auth Sub Token` приведен в разделе «[Сбор необходимых параметров](../ag/integraciya_s_google_play.md#_1)» раздела «Интеграция с Google Play».
