...
Выберите или перетащите на появившуюся страницу **Добавить сканирование** apk-файл, aab-файл (в формате Android App Bundle), ipa-файл или zip-файл, содержащий split apk сканируемого приложения.
...
* **Проект** — проект, в рамках которого будет проводиться сканирование. По умолчанию в этом поле предлагается опция **Создать проект автоматически**. Если приложение сканируется в системе впервые, можно предварительно не создавать для него проект и профиль, а использовать данную опцию. В этом случае будут автоматически созданы проект и профиль с именем сканируемого пакета, указанном в поле **Имя пакета**.
...
* **Профиль** — профиль сканирования, для которого будет проводиться сканирование. По умолчанию в этом поле предлагается опция **Создать профиль автоматически**. Данная опция в поле **Профиль** работает аналогично приведенному выше ее описанию для поля **Проект**.
...
Для запуска ручного сканирования выберите пункт основного меню **Сканы** и нажмите кнопку **Добавить сканирование** в правом верхнем углу. Выберите или перетащите на появившуюся страницу **Добавить сканирование** apk-файл, aab-файл (в формате Android App Bundle), ipa-файл или zip-файл, содержащий split apk сканируемого приложения. При загрузке файла система определит его архитектуру и отфильтрует все возможные подходящие значения в полях на появившейся странице:
